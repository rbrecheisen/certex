...
def get_batch(images, labels, params):
    ...
    for patch in range(0, params.dict['batch_size']):
        ...
        ct, gt = data_augmentation.apply_augmentations(ct,
                                                       gt,
                                                       params.dict['number_of_augmentations'])
...
def main():
    ...
    saved_model_path = 'logs/gradient_tape/' + current_time + '/saved_models/'
    saved_weights_path = 'logs/gradient_tape/' + current_time + '/saved_weights/'
    ...
    images, labels, names = utils.load_dataset(params.dict['data_path'])
    images_t = images[:, :, 10:]
    labels_t = labels[:, :, 10:]
    images_v = images[:, :, :10]
    labels_v = labels[:, :, :10]
    # images_v, labels_v, names_v = utils.load_dataset(params.dict['data_path'])
    ...
    for iteration in range(0, params.dict['num_steps'] + 1):
        ...
        if iteration % params.dict['train_eval_step'] == 0:
            ...
            early_stop = early_stopping(loss_list, min_delta=0.01, patience=20)
            if early_stop:
        ...
        if iteration % params.dict['val_eval_step'] == 0:
            ...
            with val_summary_writer.as_default():
                validation_dice = validation_accuracy(gt_batch, val_pred).numpy()
